...
exports.createDieCutting = async (req, res) => {
    try {
        const {
            //   id,
            date,
        ...
        const die = await DieCutting.create({
            qcNo,
            date,
            shift,
            serialNo,
        ...
        qc.formType = "die-cutting";
        qc.formId = die._id;
        await qc.save();
        return success_response(res, 200, "Die cutting save successfully", {
            die,
            qc,
        });
    } catch (error) {
...
exports.getAllDieMachines = async (req, res) => {
    try {
        const route = await Route.findOne({code: "9 Die Cutting"});
        ...
        return success_response(
            res,
            200,
            "All die cutting machines fetch successfully",
            getAllMachines
        );
    } catch (error) {
