...
exports.createPasting = async (req, res) => {
    try {
        const {
            //   id,
            date,
        ...
        const die = await Pasting.create({
            qcNo,
            date,
            shift,
            serialNo,
        ...
        qc.formId = die._id;
        await qc.save();
        return success_response(res, 200, "Pasting save successfully", {
            die,
            qc,
        });
    } catch (error) {
...
exports.getAllDieMachines = async (req, res) => {
    try {
        const route = await Route.findOne({code: "22 Pasting"});

        if (!route) {
            return error_response(res, 400, "Pasting route not found in db!");
        }
        ...
        return success_response(
            res,
            200,
            "Pasting  machines fetch successfully",
            getAllMachines
        );
    } catch (error) {
