...
exports.saveQuantityOrTimeForQC = async (req, res) => {
  try {
    ...
    // If quantity is provided, save quantity and current time
    if (quantity !== undefined) {
      qcData.quantity = quantity;
      qcData.quantityTime = new Date().toISOString(); // Save current time in ISO format
    }

    // If makeTime is provided, save it along with a status
    if (makeTime !== undefined) {
      qcData.makeTime = makeTime;
      qcData.makeTimeStatus = "completed"; // You can change this based on logic
    }
