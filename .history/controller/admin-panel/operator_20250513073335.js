...
//     const prodOrderNo = update.productionOrderNo;
//     const componentItemCode = update.ComponentItemCode;
...
//     const check =
//       update.totalCompletedQuantity + update.totalWastedQuantity ===
//       transferredQty;

//     if (!check) {
//       const total = update.totalCompletedQuantity + update.totalWastedQuantity;
//       if (total > transferredQty) {
//         return error_response(
//           res,
//           400,
//           `Total of completed and wasted quantity (${total}) exceeds transferred quantity (${transferredQty})`
//         );
//       } else {
//         return error_response(
//           res,
//           400,
//           `Total of completed and wasted quantity (${total}) must be equal to transferred quantity (${transferredQty})`
//         );
//       }
//     }
...
exports.updateStopProductionTime = async (req, res) => {
  try {
    ...
    if (!update) {
      return error_response(res, 400, "Production order not found!");
    }
    ...
    // Get all routes for this production order
    const allRoutes = await Job.find({
      productionOrderNo: prodOrderNo,
      ComponentItemCode: componentItemCode,
    });

    const sortedRoutes = allRoutes.sort((a, b) => a.routeNo - b.routeNo);
    const firstRouteNo = sortedRoutes[0].routeNo;
    const isFirstRoute = currentRoute === firstRouteNo;

    const total = update.totalCompletedQuantity + update.totalWastedQuantity;

    if (isFirstRoute) {
      // Allow work in progress: just don't exceed transferredQty
      if (total > transferredQty) {
        return error_response(
          res,
          400,
          `Total (completed + wasted = ${total}) exceeds transferred quantity (${transferredQty})`
        );
      }

      // If stopping, total must exactly match transferredQty
      if (startProductionTime && total !== transferredQty) {
        return error_response(
          res,
          400,
          `To stop, total (completed + wasted = ${total}) must equal transferred quantity (${transferredQty})`
        );
      }
    } else {
      const previousRoute = sortedRoutes.find(
        (r) => r.routeNo === currentRoute - 1
      );
      if (!previousRoute) {
        return error_response(res, 400, "Previous route not found!");
      }

      const previousCompletedQty = previousRoute.totalCompletedQuantity;

      if (total > previousCompletedQty) {
        return error_response(
          res,
          400,
          `Total (completed + wasted = ${total}) exceeds previous route's completed quantity (${previousCompletedQty})`
        );
      }

      if (startProductionTime && total !== previousCompletedQty) {
        return error_response(
          res,
          400,
          `To stop, total (completed + wasted = ${total}) must equal previous route's completed quantity (${previousCompletedQty})`
        );
      }
    }

    // If all checks passed, update the document
    update.startProductionTime = startProductionTime;
    update.status = "completed";
...
exports.getCompletedQtyOfPreviousRoute = async (req, res) => {
  try {
    ...
    // Get the last route (highest routeNo)
    const lastRouteJob = await Job.findOne({
      productionOrderNo: productionOrder,
      ComponentItemCode,
    }).sort({ routeNo: -1 });

    console.log("lastRouteJob", lastRouteJob);
    ...
    console.log("responseQty before UPS", responseQty);

    if (lastRouteJob.routeNo === routeNo) {
      // This is the last route, so multiply with UPS
      const UPS = lastRouteJob.U_UPS;
      if (!UPS) {
        return error_response(res, 400, "UPS not found for the last route.");
      }
...
exports.getFirstRouteForIssueToPostDataIntoSap = async (req, res) => {
  try {
    const { Quantity, id, sessionId } = req.body;

    if (!(Quantity && id && sessionId)) {
      return error_response(res, 400, "All input are required!");
    }

    const prodDetail = await Job.findOne({ _id: id }).populate(
      "productionOrderDataId"
    );
    ...
    const BaseLine = prodDetail.productionOrderDataId.baseLine;
    const BaseEntry = prodDetail.productionOrderDataId.docEntry;
...
exports.getLastRouteForCompletedToPostDataIntoSap = async (req, res) => {
  try {
    const { Quantity, id, sessionId } = req.body;
    ...
    const lastRouteNo = Math.max(...array);
    const isLastRoute = currentRoute === lastRouteNo;

    if (!isLastRoute) {
      return error_response(res, 400, "Current route is not last route!");
    }
    ...
    const issueUrl = process.env.SAP_COMPLETED_URL;
    ...
    return success_response(
      res,
      200,
      "Completed quantity  posted into sap successfully",
      sapResponse.data
    );
  } catch (error) {
    console.log(error);
