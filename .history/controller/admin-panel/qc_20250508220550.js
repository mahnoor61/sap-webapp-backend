...
exports.getQcCurrentTableData = async (req, res) => {
  try {
    ...
    const qc = await QC.find({ jobId, userId })
      .populate({
        path: "jobId",
        populate: [
          { path: "productionOrderDataId", model: "Production" },
    ...
    // .lean();
    // const productionOrderDataId = qc.map((data) => {
    //   const prodId = data.jobId.productionOrderDataId;
    //   const machineId = data.jobId.machine;
    //   await Production.findOne({_id:prodId})
    //   await Machine.findOne({_id:prodId})
    // });
    return success_response(res, 200, "Quantity save successfully", qc);
  } catch (error) {
    console.error(error);
