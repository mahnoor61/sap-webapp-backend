...
exports.getJobData = async (req, res) => {
  try {
    ...
    const job = await QC.findOne({ jobId }).populate("jobId").lean();
    ...
    let productionOrderData = null;
    if (prodId) {
      productionOrderData = await Production.findById(prodId).lean();
      productionOrderData = await Machine.findById(machineId).lean();
    }
