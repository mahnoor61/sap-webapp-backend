...
exports.getFirstRouteForIssueForMachineTo = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    const prodDetail = await Job.finOne({ _id: id });
    const currentRoute = prodDetail.routeNo;
    const prodOrderNo = prodDetail.productionOrderNo;
    const allRoutes = await Job.find({ productionOrderNo: prodOrderNo });
    ...
    return success_response(res, 200, "Route check completed", {
      isFirstRoute,
      currentRoute,
      firstRouteNo,
    });
    // return success_respon se(res, 200, " Leaderboard fetched successfully", leaderboard);
  } catch (error) {
    return error_response(res, 500, error.message);
  }
};