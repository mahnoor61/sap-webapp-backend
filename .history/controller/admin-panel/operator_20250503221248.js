...
exports.updateStopProductionTime = async (req, res) => {
  try {
    ...
    // Get all routes of this production order
    const allRoutes = await Job.find({
      productionOrderNo: prodOrderNo,
      ComponentItemCode: componentItemCode,
    });

    // Sort routes by routeNo ascending
    ...
    if (isFirstRoute) {
      ...
      // Not first route, check against previous route's completed quantity
      ...
      if (total !== previousCompletedQty) {
        return error_response(
          res,
          400,
          `Total of completed and wasted quantity (${total}) must be equal to previous route's completed quantity (${previousCompletedQty})`
        );
      }
    }
