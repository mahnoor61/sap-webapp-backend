...
const {
  getProductionOrderNos,
  getOperatorPortalData,
  getDocNumData,
} = require("../../controller/admin-panel/sapDataConnection");

const { createRoles, allRoles } = require("../../controller/admin-panel/role");
const {
  createMachine,
  allMachine,
} = require("../../controller/admin-panel/machine");
const {
  createRouteStage,
  allRoutes,
} = require("../../controller/admin-panel/routeStage");
const {
  createJob,
  allJobs,
  updateJob,
  allSingleJobs,
  destroyJob,
} = require("../../controller/admin-panel/job");
...
router.get("/all/machines", middleware, allMachine);
router.get("/all/roles", middleware, allRoles);
//route stage
router.post("/create/route", middleware, createRouteStage);
router.get("/all/routes", middleware, allRoutes);
